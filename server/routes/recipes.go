...
// TODO: fix this it doesn't work lol like it deletes a random one from the collection or something???
// idk i don't have enough brain cells rn maybe later
func (r *Client) DeleteRecipe(c *gin.Context) {
	ctx, cancel := context.WithTimeout(context.Background(), 60*time.Second)
	defer cancel()

	cookbookID := c.Param("cookbookID")

	recipeDeleted, err := r.RecipeCollection.DeleteOne(ctx, r.RecipeCollection.FindOne(ctx, bson.M{"cookbookID": cookbookID}))
	if err != nil {
	...
	stubDeleted, err := r.StubCollection.DeleteOne(ctx, r.StubCollection.FindOne(ctx, bson.M{"cookbookID": cookbookID}))
	if err != nil {
